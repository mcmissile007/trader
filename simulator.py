...
def main(cpu,models_testing):
    ...
    while (True):
        ...
        amount_invested_in_base  = sum(quote_balance) * current_rate
        ...
        if sum(base_balance) < float(model['amount_to_sos_mode'])  and enought_quote_balance:
             ...
             _sim.sim_sell_SOS(logger,local_data_base_config,currency_pair,last_candle_df.iloc[0]['close'],last_candle_df.iloc[0]['roc1'],time_frame,output_rsi,always_win,min_current_rate_benefit,max_amount_to_buy_in_base,base_balance,purchase_operations,trade_operations,quote_balance,now,sos_model_benefit,sos_games) 
             continue
        ...
        if last_candle_df.iloc[0]['rsi'] >  (output_rsi / 1000.0):
            ...
            if len(purchase_operations) > 0  and  enought_quote_balance:
                logger.debug("rsi output reached:{}".format(last_candle_df.iloc[0]['rsi']))
                logger.debug("good point to sell...")
                _sim.sim_sell (logger,local_data_base_config,currency_pair,last_candle_df.iloc[0]['close'],last_candle_df.iloc[0]['roc1'],time_frame,output_rsi,always_win,min_current_rate_benefit,max_amount_to_buy_in_base,base_balance,purchase_operations,trade_operations,quote_balance,now,games)
            else:
                pass
            ...
            if model['func']  == 0:
                if  _invest.shouldIInvest(logger,learning_df,last_candle_df,model,now,currency_pair):
                    if sum(base_balance) > model['initial_amount_to_buy_in_base']:
                        _sim.sim_buy_in_base (logger,local_data_base_config,currency_pair,last_candle_df.iloc[0]['close'],last_candle_df.iloc[0]['roc1'],time_frame,output_rsi,model,purchase_operations,trade_operations,base_balance,quote_balance,now,repurchases)
                    else:
                        pass
